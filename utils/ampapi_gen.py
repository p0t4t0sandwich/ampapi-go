...
cs_to_go = {
    ...
    "JSONRawResponse": "map[string]any",
...
class CodeGen:
    ...
    def _convert_type(self, type_name: str, optional: bool = False) -> str:
        ...
        if type_name.startswith("List<"):
            ...
            if type_name in cs_to_go:
                converted_type = cs_to_go[type_name]
            else:
    ...
    def generate_types(self) -> None:
            ...
            if type_def.SpecialType == "Enum":
                for field in type_def.Fields:
                    ...
                    if field.Description != "":
                        ...
                        field_text += "\n"
                    field_text_reversed = f"        {field.Value}:\t\"{field.Name}\",\n"

                    fields += field_text
                    fields_reversed += field_text_reversed

                if fields.endswith("\n"):
                    fields = fields[:-1]
                if fields_reversed.endswith("\n"):
                    fields_reversed = fields_reversed[:-1]

                enum_text = enum_text.replace("{TypeFields}", fields)
                enum_text = enum_text.replace("{TypeFieldsReverse}", fields_reversed)
                ...
                for field in type_def.Fields:
                    ...
                    field_description = f"{field.Name} {field.Description}"
                    if field.Optional:
                        field_description = "Optional, " + field_description
                    field_text = f"    {field.Name}\t{field.TypeName}\t`json:\"{field.Name}\"` // {field_description}\n"
                ...
                for field in type_def.Fields:
                    ...
                    if field.Optional:
                        field_description = "Optional, " + field_description
                    field_text = f"    {name}\t{field.TypeName}\t`json:\"{field.Name}\"` // {field_description}\n"
        ...
        with open("../types.go", "w", encoding="UTF-8") as file:
            file.write(text)
